...
for row in fileAsList[1:]:
    if row[stIndex] == "CERTIFIED":
        ...
        if (row[nameIndex]) in socNameDict.keys():
            ...
            socNameDict[row[nameIndex]] = 1
N = 10 #Under the context of the question to return top N. Can be modified to vary results
#Sorting the Occupation and State Dictionaries to return the top N records            
#and saving them to a new list of tuples with the desired output values.
def trueSorted(dictIn,N=10):
    """
